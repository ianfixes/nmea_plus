  ...
  describe "testing the parser" do
    ...
    context "when reading a VDM message type 4" do
      it "properly decodes the armored payload" do
        ...
        expect(parsed.ais.current_time).to eq(Time.new(2015, 12, 6, 5, 39, 12, "+00:00"))
        expect(parsed.ais.position_10m_accuracy?).to eq(true)
      ...
      it "properly decodes the armored payload libais #25" do
        ...
        expect(parsed.ais.current_time).to eq(Time.new(2010, 5, 2, 0, 0, 0, "+00:00"))
        expect(parsed.ais.position_10m_accuracy?).to eq(true)
      ...
      it "properly decodes the armored payload libais #26" do
        ...
        expect(parsed.ais.current_time).to eq(Time.new(2010, 5, 2, 23, 59, 59, "+00:00"))
        expect(parsed.ais.position_10m_accuracy?).to eq(false)
      ...
      it "properly decodes the armored payload libais #27" do
        ...
        expect(parsed.ais.current_time).to eq(Time.new(2010, 5, 2, 23, 59, 58, "+00:00"))
        expect(parsed.ais.position_10m_accuracy?).to eq(true)
    ...
    context "when dealing with VDM payload data message type 10" do
      it "properly decodes the message" do
        input = "!AIVDM,1,1,,B,:8JRSH1GTvbH,0*5A"
        parsed = @parser.parse(input)
        expect(parsed.ais.message_type).to eq(10)
        expect(parsed.ais.repeat_indicator).to eq(0)
        expect(parsed.ais.source_mmsi).to eq(564700000)
        expect(parsed.ais.destination_mmsi).to eq(367327910)
      end
    end
    ...
    context "when dealing with VDM payload data message type 11" do
      it "properly decodes the armored payload libais #105" do
        ...
        expect(parsed.ais.current_time).to eq(Time.new(2010, 4, 28, 0, 9, 56, "+00:00"))
        expect(parsed.ais.position_10m_accuracy?).to eq(true)
    ...
    context "when dealing with VDM payload data message type 15" do
      it "properly decodes the payload for a 1-mmsi-1-message interrogation" do
        input = "!AIVDM,1,1,,A,?39KQCPjNCn0D00,2*65"
        parsed = @parser.parse(input)
        expect(parsed.ais.message_type).to eq(15)
        expect(parsed.ais.repeat_indicator).to eq(0)
        expect(parsed.ais.source_mmsi).to eq(211214670)
        expect(parsed.ais.interrogation1_mmsi).to eq(211701600)
        expect(parsed.ais.interrogation1_type1).to eq(5)
        expect(parsed.ais.interrogation1_offset1).to eq(0)
        expect(parsed.ais.interrogation1_type2).to eq(nil)
        expect(parsed.ais.interrogation1_offset2).to eq(nil)
        expect(parsed.ais.interrogation2_mmsi).to eq(nil)
        expect(parsed.ais.interrogation2_type).to eq(nil)
        expect(parsed.ais.interrogation2_offset).to eq(nil)
      end
    end
